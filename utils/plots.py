...
'''
def plot_img_label(img_file, alpha=0.3):
    ...
    output_file = os.path.join('outputs', os.path.basename(img_file).replace('.png'
    , '_overlay.png'))
...
img_file = 'data/kari-road/train/images/BLD00001_PS3_K3A_NIA0276.png'
label_file = img_file.replace('images','labels')
img = cv2.imread(img_file)
label = cv2.imread(label_file,cv2.IMREAD_GRAYSCALE) # (H, W) shape
